...
import { CountSubscribers } from './count-subscribers.js';
...
export class ContextsDispatcher {
  ...
  #currentContextSubscribers: CountSubscribers;
  #nonCurrentContextsSubscribers: CountSubscribers;

  constructor() {
    this.#currentContextSubscribers = new CountSubscribers();
    ...
    this.#nonCurrentContextsSubscribers = new CountSubscribers();
  ...
  private mustFireEvent(isCurrent: boolean): boolean {
    return (
      (isCurrent && this.#currentContextSubscribers.hasSubscribers()) ||
      (!isCurrent && this.#nonCurrentContextsSubscribers.hasSubscribers())
    );
  }
  ...
  unsubscribeCurrentContext(): void {
    this.#currentContextSubscribers.unsubscribe();
  }
  ...
  unsubscribeNonCurrentContexts(): void {
    this.#nonCurrentContextsSubscribers.unsubscribe();
  }
}
